...
| Reports tab | Renderiza completions. Per-card: total chip + `.report-breakdown` inline (Svc / Acc / Fix — solo categorías con valor > 0, separadas por `\|`), Copy, Share (WhatsApp/SMS/Email/Copy), Delete. Global: Share All (`generateDailyReport()`), Delete All, DB export → `dashboard_import_{date}.json`, CSV export → `service_reports_{MM-DD-YY}.csv` |
