...
| Reports tab | Sticky `.reports-header` (top of tab): DB and CSV export buttons aligned right. Cards use `.report-card` with tap-to-select pattern: tap card body → toggles `.report-selected`, collapses all others (accordion). Per-card action `.btn-row` (Copy, Edit, Share) is `display:none` by default and shown only on `.report-selected` via CSS — no `hidden` class used (conflicts with `display:none !important`). Delete (✕) is `.report-delete-btn` positioned absolute top-right, always visible. Share expands inline share panel. Global actions (`#reports-global-actions`): Delete All (left, danger color) and Share All (right) in a space-between row; Share All expands `#share-all-panel`. DB export → `dashboard_import_{date}.json`, CSV → `service_reports_{MM-DD-YY}.csv`. |
