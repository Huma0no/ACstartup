...
**Última actualización:** Junio 2026 (rev 4)
...
| `TECH_SUPPLIED_ACCESSORIES` | array | Lista canónica de accesorios restockables: `HZ322`, `UT3000`, `Harmony`, `DAPC`, `Float Switch`, `LP Kit Lennox 1stg`, `LP Kit Lennox 2stg`, `LP Kit Goodman`, `RDS`. **Sync point:** `server.js` tiene una copia mirror de esta lista para el branch de import legacy y la query de restock. Si la lista cambia aquí, actualizar también en `server.js`. |
...
| Tab navigation | Activa/oculta panels por `data-tab`; dispara `renderJobs()`, `renderReports()` y `renderLV()` al entrar en sus respectivos tabs |
| Jobs tab | Renderiza lista agrupada por subdivisión con color auto-asignado; search por dirección; toggle expand/collapse (accordion — solo un job-item expandido a la vez; click en expandido lo colapsa; al expandir: `scrollIntoView` siempre corre, `updateJobCardHeader(job)` solo si `!_activeJob`; al colapsar: `updateJobCardHeader(null)` solo si `!_activeJob`); delete; start/resume → workspace |
...
| Workspace | 5 secciones accordion en `#workspace-form`: **Service** (`#section-service` — services + thermostat combinados), **Accessories** (`#section-accessories`), **Fixes** (`#section-fixes`), **Weigh-In** (`#section-weight-in`), **Notes** (`#section-notes`). Click en `.step-header` abre la sección y cierra las demás (un solo `.acc-open` a la vez). `.acc-done` se aplica automáticamente vía `updateAccordionSummaries()` cuando la sección tiene contenido; `.acc-state-icon` muestra `✓` / `○` en consecuencia. Event delegation desde `#workspace-form` para servicios, tstat, accesorios, fixes (standalone chips primero en `.ws-btn-grid`, luego grupos expandibles "Fixed Leaks" y "Extended LV Wire"; grupos con badge contador de seleccionados; sub-chips multi-select), weight-in (Line Config como `<select>`; auto-fill de `factoryChargeOz` desde `outdoor.FactoryCharge` y `approxAdjustOz` según revisedCharge/FactoryCharge al cargar; subcooling auto-calc: `subcoolingValue = condenserSatTemp − liquidLineTemp`, `subcoolingDeviation = abs(subcoolingValue − oemSubcoolingGoal)`), notas, fotos. **Botones de workspace:** todos los botones de selección usan `.ws-btn` (64px min-height, full-width, borde suave) + `.ws-btn-active` para seleccionado. **Service/Thermostat:** `#service-type-buttons` y `#thermostat-buttons` son contenedores grid directos (`display: grid; grid-template-columns: repeat(2, 1fr)`) — los botones `.ws-btn` son hijos directos, sin wrapper div. **Thermostat qty:** `.ws-qty-row` (4 botones 1/2/3/4+, `data-qty-select`, `grid-column: 1 / -1` para span full-width) reemplaza al qty-ctrl de increment/decrement; handler detecta `[data-qty-select]` y llama `setThermostat(tsel, qty)` directo. **Accessories:** `#accessory-buttons` contiene un único `.ws-zone-grid` (`display: grid; grid-template-columns: repeat(3, 1fr)`) con los 20 accesorios en orden fijo: UT3000, HZ322, Harmony, DAPC, E_BYPASS, BYPASS, FIN180P, DEHUM, FLOAT_SWITCH, WEIGHT_IN_DATA, ECOIL_WIRE, APRIL_AIR, FA_INTAKE, FIN6_MD, TRANE_HARNESS, RDS, LP_KIT_LENNOX_1STG, LP_KIT_LENNOX_2STG, LP_KIT_GOODMAN, OTRO. **Fixes:** standalone chips en `.ws-btn-grid` (`auto-fit minmax(100px, 1fr)`), luego grupos expandibles. **Weigh-in inputs:** `.wi-grid input, .wi-grid select` tienen `box-shadow: inset 0 2px 4px` para efecto de profundidad. |
| Generate Report | `buildCompletion()` → `generateReportText()` → `saveCompletion()` → `removeJob(id)` → limpia workspace → navega a Reports |
| Reports tab | Sticky `.reports-header` (top of tab): DB and CSV export buttons aligned right. Cards use `.report-card` with tap-to-select pattern: tap card body → toggles `.report-selected`, collapses all others (accordion). Per-card action `.btn-row` (Copy, Edit, Share) is `display:none` by default and shown only on `.report-selected` via CSS — no `hidden` class used (conflicts with `display:none !important`). Delete (✕) is `.report-delete-btn` positioned absolute top-right, always visible. Share expands inline share panel. Global actions (`#reports-global-actions`): Delete All (left, danger color) and Share All (right, `flex: 1` — full width) in a space-between row; Share All expands `#share-all-panel`. DB export → `dashboard_import_{date}.json`, CSV → `service_reports_{MM-DD-YY}.csv`. |
...
- `.job-item.expanded`: `scroll-margin-top: calc(var(--header-h) + 36px + var(--nav-h) + 8px)` — usa el valor mayor (con active-job-row) como default seguro para que `scrollIntoView` aterrice debajo del header + nav
...
| 21 | ✅ Test data scaffold removed from `index.html` — 3 static `<li>` job items deleted; `#jobs-list` remains empty, populated at runtime by `renderJobs()`. | index.html |
| 22 | ✅ Emoji removed from live JS-rendered UI strings — buttons, labels, chips in `app.js` and `workspace.js`; `lv-uc-icon` div removed from `lv.js`. `✓`, `○`, `✕`, `📷` intentionally preserved. | src/app.js / src/workspace.js / src/lv.js |
| 23 | Settings price fields are static HTML in `index.html` — should be dynamically generated by `renderSettingsModal()` from the data catalog (`SERVICES`, `ACCESSORIES`, `FIXES` in `data.js`). Static labels risk silent drift when enum keys change. Refactor candidate for a future session. | index.html / src/app.js / src/data.js |
| 24 | ✅ Custom Reports Equipment & Accessories restock — query rewritten to `WHERE tech_supplied = 1 AND category IN ('Accessory','Thermostat')`, fully dynamic (no hardcoded checklist). Legacy import branch: `acc.techSupplied !== false` replaced by `TECH_SUPPLIED_ACCESSORIES.includes(acc.name)`. New-format thermostat branch added (reads `job.selectedThermostat`). Old `CR_EQUIPMENT_GROUPS` checklist and related JS removed from dispatch.html. | server.js / dispatch.html |
| 25 | ✅ Dispatch Note chip in PWA — Manual History field in Dispatch New Job form exports as `dispatchNote`; PWA renders it as a second "Dispatch Note" chip alongside History chip. History count badge removed. | dispatch.html / src/app.js |
